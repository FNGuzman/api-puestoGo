...
export class AdminAuthGuard implements CanActivate {
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!authSecret) {
      throw new UnauthorizedException(
        'JWT_SECRET no configurado. No se pueden validar tokens de admin.',
      );
    }
    ...
    try {
      ...
      if (!payload.id) {
        throw new ForbiddenException(
          'Este endpoint solo es accesible para administradores con token de AUTH externa',
        );
      }
      ...
      throw new UnauthorizedException(
        `Error al procesar el token: ${error.message}`,
      );
    }
  }
}
 ...
 * Valida que el token sea de AUTH externa Y que el rolId sea el id especifico de administrador del sistema auth
 */
@Injectable()
export class AdminRoleAuthGuard implements CanActivate {
  canActivate(context: ExecutionContext): boolean {
    ...
    try {
      ...
      // Validar que el rolId sea 56 (Administrador de Oficios)
      const rolId = Number(payload.rolId);
      if (!rolId || rolId !== AUTH_CONSTANTS.ADMIN_ROLE_ID) {
        throw new ForbiddenException(
          `Este endpoint requiere permisos de administrador (rolId: ${AUTH_CONSTANTS.ADMIN_ROLE_ID})`,
        );
      }
