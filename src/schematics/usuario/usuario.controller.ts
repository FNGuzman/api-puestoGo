...
export class UsuarioController {
  ...
  @ApiOperation({ 
    ...
    description: 'Busca usuarios con filtros opcionales'
  })
  @ApiOkResponse({ 
  ...
  async update(
    ...
    return await this.usuarioService.update(id, updateUsuarioRequestDto, req.user, file);
  }
  ...
  @ApiOperation({ 
    ...
    description: 'Elimina un usuario por su ID, solo se puede eliminar si es admin o el usuario es el mismo'
  })
  @ApiParam({ name: 'id', required: true, description: 'ID del Usuario' })
  ...
  @ApiNotFoundResponse({ description: 'No se encontró el usuario' })
  @ApiUnauthorizedResponse({ description: 'No autorizado' })
  async delete(@Request() req: any, @Param('id', ParseIntPipe) id: number): Promise<string> {
