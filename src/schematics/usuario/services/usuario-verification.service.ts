...
export class UsuarioVerificationService {
  ...
  async verifyCode(usuarioId: number, codigo: string): Promise<UsuarioDTO> {
    const usuario = await this.getEntity.findById(Usuario, usuarioId, { persona: true });
    ...
    if (!usuario.codigoVerificacionEmail || usuario.codigoVerificacionEmail !== codigo) {
      this.errorHandler.throwBadRequest(ERRORS.VALIDATION.INVALID_INPUT, 'Codigo de verificacion invalido');
    }
    ...
    if (!expiraEnDate || isNaN(expiraEnDate.getTime()) || now > expiraEnDate) {
      this.errorHandler.throwBadRequest(ERRORS.VALIDATION.INVALID_INPUT, 'El codigo de verificacion expiro. Solicita uno nuevo.');
    }
  ...
  async resendVerificationCode(usuarioId: number): Promise<{ email: string; codigo: string }> {
    const usuario = await this.getEntity.findById(Usuario, usuarioId);
    if (usuario.emailVerificado) {
      this.errorHandler.throwBadRequest(ERRORS.VALIDATION.INVALID_INPUT, 'El correo ya esta verificado');
    }
