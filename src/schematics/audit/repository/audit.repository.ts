...
export class AuditoriaRepository extends Repository<AuditLog> {
  ...
  async search(request: SearchAuditLogDto): Promise<PageDto<AuditLog>> {
    const queryBuilder: SelectQueryBuilder<AuditLog> =
      this.createQueryBuilder('audit_log');
    ...
    if (request.method) {
      queryBuilder.andWhere('audit_log.method = :method', {
        method: request.method,
      });
    }

    if (request.path) {
      queryBuilder.andWhere('audit_log.path LIKE :path', {
        path: `%${request.path}%`,
      });
    }

    if (request.requestId) {
      queryBuilder.andWhere('audit_log.request_id = :requestId', {
